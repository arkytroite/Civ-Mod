...
local g_IdeologyBackgrounds = {
		...
		POLICY_BRANCH_REACTIONARY = "SocialPoliciesReactionary.dds",
...
function GetPipe(branchType)
	...
	elseif branchType == "POLICY_BRANCH_ARDL_DIPLOMACY" then
		controlTable = g_FreedomPipeManager:GetInstance();
	elseif branchType == "POLICY_BRANCH_ARDL_SECURITY" then
		controlTable = g_OrderPipeManager:GetInstance();
	elseif branchType == "POLICY_BRANCH_ARDL_AUTHORITY" then
...
function Init()
	...
	while policyInfo ~= nil do
		...
		if (iBranch ~= nil) then
			...
			elseif iBranch == "POLICY_BRANCH_ARDL_DIPLOMACY" then
				controlTable = g_FreedomInstanceManager:GetInstance();
			elseif iBranch == "POLICY_BRANCH_ARDL_SECURITY" then
				controlTable = g_OrderInstanceManager:GetInstance();
			elseif iBranch == "POLICY_BRANCH_ARDL_AUTHORITY" then
